...
class _ReservaMesaTrabajadorState extends State<ReservaMesaTrabajador>
  ...
  void initState() {
    ...
    // Tab controller con 1 tab: el panel del trabajador solo crea reservas
    // (las gestiona desde Gestión de Reservas). No mostramos "Mis Reservas"
    // porque el trabajador reserva para clientes, no para sí mismo.
    _tabController = TabController(length: 1, vsync: this);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      // El trabajador reserva PARA OTROS — no autocompletamos su nombre.
  ...
  void _mostrarConfirmacion(Reserva r) {
    showModalBottomSheet(
      ...
      builder: (_) => _ConfirmacionSheet(
        ...
        // Tras confirmar, cierra el sheet y vuelve a Gestión de Reservas
        // (que recarga la lista al volver del push). El trabajador ya tiene
        // la reserva creada y puede seguir gestionando desde el panel.
        onVerReservas: () {
          Navigator.pop(context); // sheet
          if (mounted) Navigator.pop(context); // pantalla → Gestión
        },
      ),
    );
  }
        ...
        children: [
          ...
          SafeArea(
            child: Column(
              children: [
                _buildAppBar(),
                Expanded(child: _buildTabNuevaReserva()),
              ],
            ),
          ),
        ],
